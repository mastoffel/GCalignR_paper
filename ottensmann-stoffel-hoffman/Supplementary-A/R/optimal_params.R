...
optimal_params <- function(data,rt_col_name,conc_col_name,max_diff_peak2mean,min_diff_peak2peak) {
    ...
    for (i in max_diff_peak2mean) {
        for (n in min_diff_peak2peak) {
            out <- append(out,list(align_chromatograms(data = data,conc_col_name = conc_col_name,rt_col_name = rt_col_name,max_diff_peak2mean = i,min_diff_peak2peak = n,max_linear_shift = 0.01)))
